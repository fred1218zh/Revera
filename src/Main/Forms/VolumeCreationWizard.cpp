		...
		case Step::VolumeSize:
			{
				...
				else if (SelectedVolumeType == VolumeType::Hidden)
				{
					...
					pageText = LangString["SIZE_HELP_HIDDEN_VOL"] + L"\n\n" + _("Please note that if your operating system does not allocate files from the beginning of the free space, the maximum possible hidden volume size may be much smaller than the size of the free space on the outer volume. This not a bug in Revera but a limitation of the operating system.");
